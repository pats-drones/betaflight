...
static void taskUpdateRxMain(timeUs_t currentTimeUs)
{
    ...
    if (!ARMING_FLAG(ARMED) && rcData[AUX2] > PATS_DIRECT_SPIN_MOTOR_MIN && rcData[AUX2] < PATS_DIRECT_SPIN_MOTOR_REVERSED_MAX && (rxIsReceivingSignal()|| batteryisunsafeget())) {
    //if (!ARMING_FLAG(ARMED) && rcData[AUX2] > PATS_DIRECT_SPIN_MOTOR_MIN && rcData[AUX2] < PATS_DIRECT_SPIN_MOTOR_REVERSED_MAX) {
            ...
            if (rcData[ROLL]<1000)
                rcData[ROLL]=1000;
            else if (rcData[ROLL]>2000)
                rcData[ROLL]=2000;

            if (rcData[PITCH]<1000)
                rcData[PITCH]=1000;
            else if (rcData[PITCH]>2000)
                rcData[PITCH]=2000;
